const cardData = [
    [
      {
        word: 'fly', // actions
        translation: 'летать',
        image: 'images/actions/fly.jpg',
        audioSrc: 'audio/cry.mp3',
      },
      {
        word: 'hug',
        translation: 'обнимать',
        image: 'images/actions/hug.jpg',
        audioSrc: 'audio/dance.mp3',
      },
      {
        word: 'jump',
        translation: 'прыгать',
        image: 'images/actions/jump.jpg',
        audioSrc: 'audio/dive.mp3',
      },
      {
        word: 'laugh',
        translation: 'смеяться',
        image: 'images/actions/laugh.jpg',
        audioSrc: 'audio/draw.mp3',
      },
      {
        word: 'open',
        translation: 'открывать',
        image: 'images/actions/open.jpg',
        audioSrc: 'audio/fish.mp3',
      },
      {
        word: 'play',
        translation: 'играть',
        image: 'images/actions/play.jpg',
        audioSrc: 'audio/fly.mp3',
      },
      {
        word: 'run',
        translation: 'бегать',
        image: 'images/actions/run.jpg',
        audioSrc: 'audio/hug.mp3',
      },
      {
        word: 'sing',
        translation: 'петь',
        image: 'images/actions/sing.jpg',
        audioSrc: 'audio/jump.mp3',
      },
    ],
    [
      {
        word: 'cat', // animal 1
        translation: 'кот',
        image: 'images/animals (set 1)/cat.jpg',
        audioSrc: 'audio/open.mp3',
      },
      {
        word: 'chick',
        translation: 'цыпленок',
        image: 'images/animals (set 1)/chick.jpg',
        audioSrc: 'audio/play.mp3',
      },
      {
        word: 'chicken',
        translation: 'курица',
        image: 'images/animals (set 1)/chicken.jpg',
        audioSrc: 'audio/point.mp3',
      },
      {
        word: 'dog',
        translation: 'собака',
        image: 'images/animals (set 1)/dog.jpg',
        audioSrc: 'audio/ride.mp3',
      },
      {
        word: 'dolphin',
        translation: 'дельфин',
        image: 'images/animals (set 1)/dolphin.jpg',
        audioSrc: 'audio/run.mp3',
      },
      {
        word: 'rabbit',
        translation: 'кролик',
        image: 'images/animals (set 1)/rabbit.jpg',
        audioSrc: 'audio/sing.mp3',
      },
      {
        word: 'sheep',
        translation: 'овца',
        image: 'images/animals (set 1)/sheep.jpg',
        audioSrc: 'audio/skip.mp3',
      },
      {
        word: 'turtle',
        translation: 'черепаха',
        image: 'images/animals (set 1)/turtle.jpg',
        audioSrc: 'audio/swim.mp3',
      },
    ],
    [
      {
        word: 'bird', // animals 2
        translation: 'птица',
        image: 'images/animals (set 2)/bird.jpg',
        audioSrc: 'audio/cat.mp3',
      },
      {
        word: 'fish',
        translation: 'рыба',
        image: 'images/animals (set 2)/fish.jpg',
        audioSrc: 'audio/chick.mp3',
      },
      {
        word: 'frog',
        translation: 'лягушка',
        image: 'images/animals (set 2)/frog.jpg',
        audioSrc: 'audio/chicken.mp3',
      },
      {
        word: 'giraffe',
        translation: 'жираф',
        image: 'images/animals (set 2)/giraffe.jpg',
        audioSrc: 'audio/dog.mp3',
      },
      {
        word: 'horse',
        translation: 'лошадь',
        image: 'images/animals (set 2)/horse.jpg',
        audioSrc: 'audio/horse.mp3',
      },
      {
        word: 'lion',
        translation: 'лев',
        image: 'images/animals (set 2)/lion.jpg',
        audioSrc: 'audio/pig.mp3',
      },
      {
        word: 'mouse',
        translation: 'мышь',
        image: 'images/animals (set 2)/mouse.jpg',
        audioSrc: 'audio/rabbit.mp3',
      },
      {
        word: 'pig',
        translation: 'свинья',
        image: 'images/animals (set 2)/pig.jpg',
        audioSrc: 'audio/sheep.mp3',
      },
    ],
    [
      {
        word: 'skirt', // clothes
        translation: 'юбка',
        image: 'images/clothes/skirt.jpg',
        audioSrc: 'audio/skirt.mp3',
      },
      {
        word: 'pants',
        translation: 'брюки',
        image: 'images/clothes/pants.jpg',
        audioSrc: 'audio/pants.mp3',
      },
      {
        word: 'blouse',
        translation: 'блузка',
        image: 'images/clothes/blouse.jpg',
        audioSrc: 'audio/blouse.mp3',
      },
      {
        word: 'dress',
        translation: 'платье',
        image: 'images/clothes/dress.jpg',
        audioSrc: 'audio/dress.mp3',
      },
      {
        word: 'boot',
        translation: 'ботинок',
        image: 'images/clothes/boot.jpg',
        audioSrc: 'audio/boot.mp3',
      },
      {
        word: 'shirt',
        translation: 'рубашка',
        image: 'images/clothes/shirt.jpg',
        audioSrc: 'audio/shirt.mp3',
      },
      {
        word: 'coat',
        translation: 'пальто',
        image: 'images/clothes/coat.jpg',
        audioSrc: 'audio/coat.mp3',
      },
      {
        word: 'shoes',
        translation: 'туфли',
        image: 'images/clothes/shoes.jpg',
        audioSrc: 'audio/shoes.mp3',
      },
    ],
    [
      {
        word: 'sad', // emotions
        translation: 'грустный',
        image: 'images/emotions/sad.jpg',
        audioSrc: 'audio/sad.mp3',
      },
      {
        word: 'angry',
        translation: 'сердитый',
        image: 'images/emotions/angry.jpg',
        audioSrc: 'audio/angry.mp3',
      },
      {
        word: 'happy',
        translation: 'счастливый',
        image: 'images/emotions/happy.jpg',
        audioSrc: 'audio/happy.mp3',
      },
      {
        word: 'tired',
        translation: 'уставший',
        image: 'images/emotions/tired.jpg',
        audioSrc: 'audio/tired.mp3',
      },
      {
        word: 'surprised',
        translation: 'удивлённый',
        image: 'images/emotions/surprised.jpg',
        audioSrc: 'audio/surprised.mp3',
      },
      {
        word: 'scared',
        translation: 'испуганный',
        image: 'images/emotions/scared.jpg',
        audioSrc: 'audio/scared.mp3',
      },
      {
        word: 'smile',
        translation: 'улыбка',
        image: 'images/emotions/smile.jpg',
        audioSrc: 'audio/smile.mp3',
      },
      {
        word: 'cry',
        translation: 'плач',
        image: 'images/emotions/cry.jpg',
        audioSrc: 'audio/laugh.mp3',
      },
    ],
    [
      {
        word: 'apple', //  fruits
        translation: 'яблоко',
        image: 'images/fruits/apple.jpg',
        audioSrc: 'audio/bird.mp3',
      },
      {
        word: 'avocado',
        translation: 'авокадо',
        image: 'images/fruits/avocado.jpg',
        audioSrc: 'audio/fish.mp3',
      },
      {
        word: 'banana',
        translation: 'банан',
        image: 'images/fruits/banana.jpg',
        audioSrc: 'audio/frog.mp3',
      },
      {
        word: 'lemon',
        translation: 'лемон',
        image: 'images/fruits/lemon.jpg',
        audioSrc: 'audio/giraffe.mp3',
      },
      {
        word: 'orange',
        translation: 'апельсин',
        image: 'images/fruits/orange.jpg',
        audioSrc: 'audio/lion.mp3',
      },
      {
        word: 'peach',
        translation: 'персик',
        image: 'images/fruits/peach.jpg',
        audioSrc: 'audio/mouse.mp3',
      },
      {
        word: 'pear',
        translation: 'груша',
        image: 'images/fruits/pear.jpg',
        audioSrc: 'audio/turtle.mp3',
      },
      {
        word: 'pineapple',
        translation: 'ананас',
        image: 'images/fruits/pineapple.jpg',
        audioSrc: 'audio/dolphin.mp3',
      },
    ],
    [
      {
        word: 'biscuits', // sweets
        translation: 'печенье',
        image: 'images/sweets/biscuits.jpg',
        audioSrc: 'audio/sad.mp3',
      },
      {
        word: 'cake',
        translation: 'торт',
        image: 'images/sweets/cake.jpg',
        audioSrc: 'audio/angry.mp3',
      },
      {
        word: 'candy',
        translation: 'конфета',
        image: 'images/sweets/candy.jpg',
        audioSrc: 'audio/happy.mp3',
      },
      {
        word: 'chocolate',
        translation: 'шоколад',
        image: 'images/sweets/chocolate.jpg',
        audioSrc: 'audio/tired.mp3',
      },
      {
        word: 'sugar',
        translation: 'сахар',
        image: 'images/sweets/sugar.jpg',
        audioSrc: 'audio/surprised.mp3',
      },
      {
        word: 'honey',
        translation: 'мед',
        image: 'images/sweets/honey.jpg',
        audioSrc: 'audio/scared.mp3',
      },
      {
        word: 'ice cream',
        translation: 'мороженое',
        image: 'images/sweets/icecream.jpg',
        audioSrc: 'audio/smile.mp3',
      },
      {
        word: 'jam',
        translation: 'варенье',
        image: 'images/sweets/jam.jpg',
        audioSrc: 'audio/laugh.mp3',
      },
    ],
    [
      {
        word: 'cabbage', // vegetables
        translation: 'капуста',
        image: 'images/vegetables/cabbage.jpg',
        audioSrc: 'audio/sad.mp3',
      },
      {
        word: 'carrots',
        translation: 'морковь',
        image: 'images/vegetables/carrots.jpg',
        audioSrc: 'audio/angry.mp3',
      },
      {
        word: 'cucumber',
        translation: 'огурец',
        image: 'images/vegetables/cucumber.jpg',
        audioSrc: 'audio/happy.mp3',
      },
      {
        word: 'eggplant',
        translation: 'баклажан',
        image: 'images/vegetables/eggplant.jpg',
        audioSrc: 'audio/tired.mp3',
      },
      {
        word: 'onion',
        translation: 'лук',
        image: 'images/vegetables/onion.jpg',
        audioSrc: 'audio/surprised.mp3',
      },
      {
        word: 'pepper',
        translation: 'перец',
        image: 'images/vegetables/pepper.jpg',
        audioSrc: 'audio/scared.mp3',
      },
      {
        word: 'potato',
        translation: 'картофель',
        image: 'images/vegetables/potato.jpg',
        audioSrc: 'audio/smile.mp3',
      },
      {
        word: 'tomato',
        translation: 'помидор',
        image: 'images/vegetables/tomato.jpg',
        audioSrc: 'audio/laugh.mp3',
      },
    ],
  ];
  
  export default cardData;